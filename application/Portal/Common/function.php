...
function sp_sql_posts_paged($tag, $pagesize = 20, $pagetpl = '{first}{prev}{liststart}{list}{listend}{next}{last}', $cond = [ ])
{
    $where = array();
    $tag = sp_param_lable($tag);
    $field = !empty($tag['field']) ? $tag['field'] : '*';
    ...
    //根据参数生成查询条件
//    dump($cond);
    if (isset($cond['region'])) {
        $where['post_region'] = array( 'like', $cond['region'] . '%' );
    }
    if (isset($cond['year'])) {
        $where['post_diedate'] = array( 'like', $cond['year'] . '%' );
    }
    $where['status'] = array( 'eq', 1 );
    $where['post_status'] = array( 'eq', 1 );
