 ...
 */
    ...
    class ChinaCityPlugin extends Plugin{
        ...
        public function install(){
            ...
            //读取插件sql文件
            $sqldata = file_get_contents('http://'.$_SERVER['HTTP_HOST'].__ROOT__.'/plugins/'.$this->info['name'].'/install.sql');
        ...
        public function uninstall(){
            //读取插件sql文件
            $sqldata = file_get_contents('http://'.$_SERVER['HTTP_HOST'].__ROOT__.'/plugins/'.$this->info['name'].'/uninstall.sql');
            $sqlFormat = $this->sql_split($sqldata, C('DB_PREFIX'));
            $counts = count($sqlFormat);
        ...
        //获取插件所需的钩子是否存在
        public function getisHook($str, $hooks, $msg=''){
            $hook_mod = M('plugins');
            $where['name'] = $str;
        ...
        public function sql_split($sql, $tablepre) {
            $tablepre=$tablepre.PLUGIN;
            if ($tablepre != "thinkcmf_")
                $sql = str_replace("thinkcmf_", $tablepre, $sql);
